...
public class primoCarattere {
    public static void main(String[] args) {
        ...
        for(int i = 0; i < n; i++){
            ...
            if(primo.containsKey(c)){ // se il carattere è nel hasmap il valore della chiave diventa falso
                primo.put(c, false);
            }else{
                primo.put(c, true);
            }
        }
        for(int i = 0;  i < n; i++){ // se il valore è true vuoldire che c non è ripetuto
            char c = parola.charAt(i);
            if(primo.get(c) == true){
